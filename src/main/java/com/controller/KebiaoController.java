    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(kebiao !=null){
            ...
            BeanUtils.copyProperties( kebiao , view );//把实体数据重构到view中
                ...
                //级联表
                KechengEntity kecheng = kechengService.selectById(kebiao.getKechengId());
                if(kecheng != null){
    ...
    */
    ...
    public R save(@RequestBody KebiaoEntity kebiao, HttpServletRequest request){
        ...
        Wrapper<KebiaoEntity> queryWrapper = new EntityWrapper<KebiaoEntity>()
            .eq("kecheng_id", kebiao.getKechengId())
            .eq("xingqi_types", kebiao.getXingqiTypes())
            .eq("jie_types", kebiao.getJieTypes())
            .eq("kebiao_shijian", kebiao.getKebiaoShijian())
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        KebiaoEntity kebiaoEntity = kebiaoService.selectOne(queryWrapper);
        if(kebiaoEntity==null){
            kebiao.setInsertTime(new Date());
...
//                            kebiaoEntity.setKechengId(Integer.valueOf(data.get(0)));   //课程 要改的
//                            kebiaoEntity.setXingqiTypes(Integer.valueOf(data.get(0)));   //星期 要改的
//                            kebiaoEntity.setJieTypes(Integer.valueOf(data.get(0)));   //第几节 要改的
//                            kebiaoEntity.setKebiaoShijian(data.get(0));                    //上课时间 要改的
//                            kebiaoEntity.setKebiaoContent("");//详情和图片
//                            kebiaoEntity.setInsertTime(date);//时间
